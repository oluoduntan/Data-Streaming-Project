...
def insert_data(session, **kwargs):
    ...
    try:
        session.execute(
            ...
            VALUES ({user_id}, {first_name}, {last_name}, {gender}, {address}, {postcode}, 
            {email}, {username}, {dob}, {registered_date}, {phone}, {picture})
            """
        )
        logging.info(f"Data inserted for {first_name} {last_name}")
    except Exception as e:
        logging.error(f"Data couldn't be inserted due to exception {e}")
...
def connect_to_kafka(spark_conn):
    ...
    try:
        ...
        logging.warning(f"kafka dataframe cound not be created because: {e}")
...
def create_cassandra_connection():
    try:
        ...
        logging.error(f"Cound not create cassandra connection due to {e}")
        return None
